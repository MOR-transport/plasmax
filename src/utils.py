...
``save_fig(filename, fig, ...)``
    ...
    * Multiple subplots → absolute ``at={(x cm, y cm)}`` positioning
    ...
    for mappable in list(ax.collections) + list(ax.images):
        ...
        if cbar is not None:
            ...
            if width.endswith("cm"):
                cbar_w = f"{frac * float(width[:-2]):.4f}cm"
            else:
                # width is a LaTeX length like \figurewidth — TeX allows
                # decimal multipliers: 0.0625\figurewidth is valid.
                cbar_w = f"{frac:.4f}{width}"
    ...
    *,
    ...
    for ax in data_axes:
        if single:
            ...
            pos = ax.get_position()
            ax_x_cm = pos.x0     * fig_w_in * 2.54
            ax_y_cm = pos.y0     * fig_h_in * 2.54
            ax_w_cm = pos.width  * fig_w_in * 2.54
            ax_h_cm = pos.height * fig_h_in * 2.54
            at_str = f"({ax_x_cm:.4f}cm, {ax_y_cm:.4f}cm)"
            w_str  = f"{ax_w_cm:.4f}cm"
            h_str  = f"{ax_h_cm:.4f}cm"
